...
const getUsers = async page => {
  return [...(await page.$$('main div div + div ul li'))]
}
...
const mapUsers = async page => {
  ...
  for (const user of users) {
    const {
      _remoteObject: { value: fullName },
    } = await page.evaluateHandle(user => user.querySelectorAll('div a span')[1]?.innerText, user)
    const {
      _remoteObject: { value: profileHref },
    } = await page.evaluateHandle(user => user.querySelectorAll('div span span a')[0]?.getAttribute('href'), user)
    ...
    if ((fullName, profileHref)) {
      ...
      await axios
        .get(`https://api.genderize.io/?name=${extractNameFromFullName(fullName)}`)
        .then(res => {
